...
- 维护顶部展示区标签。每个标签对应一个独立 `WatchTreeWorkspace.root`；创建、删除、重命名和切换标签都会保存到 `watch-tree.json`。用户点击标签切换展示区，双击标签重命名，点击 `+` 会立即创建一个自动命名的展示区，点击标签 `×` 删除展示区；至少保留一个展示区。标签切换只更新当前展示区并重绘脑图，不主动拉取行情，避免切换卡顿；行情更新仍由定时轮询或手动“刷新行情”触发。
