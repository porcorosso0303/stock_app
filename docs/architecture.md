...
板块强度指数算法用于分类节点，输入是该分类下所有股票的标准化 quote。算法分三层：

```text
混合幅度分 = 0.45 * 涨跌停相对幅度分 + 0.55 * 绝对涨跌幅强度分
基础强弱分 = (1 - 扩散权重) * 混合幅度分 + 扩散权重 * 涨跌家数扩散分
...
```text
涨跌停相对幅度分 = 100 * average(clamp(changePercent / limitRate, -1, 1))
绝对涨跌幅强度分 = 100 * average(tanh(changePercent / 4))
涨跌家数扩散分 = 100 * (上涨家数 - 下跌家数) / 有行情股票数
扩散权重 = 0.25 + 0.15 * sqrt(min(有行情股票数, 20) / 20)
涨跌停事件冲击分 = clamp(12 * sign(净涨跌停数) + 80 * 净涨跌停数 / 有行情股票数, -35, +35)
净涨跌停数 = 涨停家数 - 跌停家数
```

混合幅度分同时保留涨跌停制度差异和绝对涨跌幅差异：涨跌停相对幅度分避免 20cm、30cm 股票被简单等同于 10cm 股票；绝对涨跌幅强度分使用 `tanh(changePercent / 4)`，让 `-3%` 与 `-5%` 这类常见日内差异更敏感，同时避免大幅波动无限放大。扩散权重随样本数增加而提高，小分类不会因为“全部上涨/全部下跌”直接被涨跌家数项过度支配，大分类仍能体现上涨/下跌家数的扩散意义。
...
- 为每个节点输出 `data-watch-connector-trend`，作为父节点到该目标节点连接线的颜色依据。股票节点使用自身涨跌幅；分类节点使用递归平均涨跌幅；上涨为 `positive`，下跌为 `negative`，平盘或无行情为 `neutral`。
...
顶部盯盘状态栏必须用 `数据日期` 下拉框显示当前实际展示的 `tradingDate`。下拉框右侧是红色错误提示区，用于显示所选日期拉取失败、无行情或部分股票无行情等信息；每次开始新的行情拉取前必须清空旧错误。用户切换 `数据日期` 下拉框时，renderer 用所选日期请求行情；本地没有该日期完整缓存时由 main 侧拉取并写入缓存。用户手动选择日期后，renderer 会锁定该日期，定时轮询和后台最近交易日校验都不能把下拉框自动切回最新交易日。
...
当 provider 返回的数据没有任何可用分时走势时，即使 quote 快照里有涨跌幅，也不能写入 `watch-quotes-cache.json`。盯盘脑图以股票为单位，把 quote 与同交易日 trend 视为一个完整行情快照：只有 quote 含有效涨跌幅且 trend 含有效分时点时，新快照才能替换旧快照。部分股票请求失败时，`WatchMarketCacheStore.write()` 按 `secid` 合并同交易日数据，失败或缺失的股票保留原有完整快照；不同展示区分批写入同一交易日缓存时也保留其他展示区股票，不能整日覆盖。Renderer 使用相同规则合并当前帧、返回的同日历史缓存和本次行情；优先级依次为本次完整快照、当前同日完整快照、历史同日完整快照。错误仍显示在顶部错误区，但瞬时 `ERR_EMPTY_RESPONSE`、超时或部分失败不能把已展示的股票清成“暂无行情”。该回退严格限制在同一 `tradingDate`，不能用旧交易日走势冒充当前交易日。
